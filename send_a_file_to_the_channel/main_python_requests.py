...
def main():
    ...
    if file_is_not_empty(upload_file):
        uploaded_file_link = files_upload(headers, channel_id, upload_file, upload_file_title)
        if uploaded_file_link:
            message = f'<!channel> Hello Guys.\nGlad to see you again. this is for you.\n<{uploaded_file_link}|{upload_file_title}>'
            result = chat_post_message(headers, channel_id, message)
