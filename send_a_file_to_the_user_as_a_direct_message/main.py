...
def main():
    ...
    if chat_id:
        if file_is_not_empty(upload_file):
            uploaded_file_link = upload_files_to_channel(bot_token, chat_id, upload_file, upload_file_title)
            if uploaded_file_link:
                ...
                message = f'Hi~! <@{user_id}> Here is for you.\n' \
                          f'<{uploaded_file_link}|{upload_file_title}>'
